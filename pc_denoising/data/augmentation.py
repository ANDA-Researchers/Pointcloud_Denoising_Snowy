...
def cal_iou2d(box1_corner, box2_corner):
    ...
    box1_corner = (
        (cfg.W, cfg.H)
        - (box1_corner - (cfg.xrange[0], cfg.yrange[0])) / (cfg.vw, cfg.vh)
    ).astype(np.int32)
    box2_corner = (
        (cfg.W, cfg.H)
        - (box2_corner - (cfg.xrange[0], cfg.yrange[0])) / (cfg.vw, cfg.vh)
    ).astype(np.int32)
    ...
    if choice >= 7:
        for idx in range(len(gt_box3d_corner)):
            ...
            while is_collision and _count < 100:
                ...
                # check collision
                tmp = box_transform(gt_box3d_corner[[idx]], t_x, t_y, t_z, t_rz)
                is_collision = False
