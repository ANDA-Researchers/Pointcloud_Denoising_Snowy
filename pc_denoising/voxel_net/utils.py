...
def draw_lidar(lidar, is_grid=False, is_axis=True, is_top_region=True, fig=None):
    ...
    if is_grid:
        ...
        fov = np.array(
            [  ##<todo> : now is 45 deg. use actual setting later ...
                [20.0, 20.0, 0.0, 0.0],
                [20.0, -20.0, 0.0, 0.0],
            ],
            dtype=np.float64,
    ...
    )  # 2.0909996 , -1.04700089, -2.03249991
...
def draw_rgb_projections(
    image, projections, color=(255, 255, 255), thickness=2, darker=1
):
...
def draw_rects(image, rects, color=(255, 255, 255), thickness=1, darken=1):
    ...
    for rect in rects:
        tup0, tup1 = [
            _quantize_coords(*tup) for tup in list(zip(rect[0::2], rect[1::2]))
        ]
...
def anchors_center_to_corner(anchors):
    ...
    for i in range(N):
        ...
        h, w, l = anchor[3:6]
        rz = anchor[-1]
        Box = np.array([[-l / 2, -l / 2, l / 2, l / 2], [w / 2, -w / 2, -w / 2, w / 2]])
        # re-create 3D bounding box in velodyne coordinate system
        rotMat = np.array([[np.cos(rz), -np.sin(rz)], [np.sin(rz), np.cos(rz)]])
        velo_box = np.dot(rotMat, Box)
...
def box3d_corner_to_center_batch(box3d_corner):
    ...
    h = abs(
        np.mean(box3d_corner[:, 4:, 2] - box3d_corner[:, :4, 2], axis=1, keepdims=True)
    )

    w = (
        np.sqrt(
            np.sum(
                (box3d_corner[:, 0, [0, 1]] - box3d_corner[:, 1, [0, 1]]) ** 2,
                axis=1,
                keepdims=True,
            )
        )
        + np.sqrt(
            np.sum(
                (box3d_corner[:, 2, [0, 1]] - box3d_corner[:, 3, [0, 1]]) ** 2,
                axis=1,
                keepdims=True,
            )
        )
        + np.sqrt(
            np.sum(
                (box3d_corner[:, 4, [0, 1]] - box3d_corner[:, 5, [0, 1]]) ** 2,
                axis=1,
                keepdims=True,
            )
        )
        + np.sqrt(
            np.sum(
                (box3d_corner[:, 6, [0, 1]] - box3d_corner[:, 7, [0, 1]]) ** 2,
                axis=1,
                keepdims=True,
            )
        )
    ...
    l = (
        np.sqrt(
            np.sum(
                (box3d_corner[:, 0, [0, 1]] - box3d_corner[:, 3, [0, 1]]) ** 2,
                axis=1,
                keepdims=True,
            )
        )
        + np.sqrt(
            np.sum(
                (box3d_corner[:, 1, [0, 1]] - box3d_corner[:, 2, [0, 1]]) ** 2,
                axis=1,
                keepdims=True,
            )
        )
        + np.sqrt(
            np.sum(
                (box3d_corner[:, 4, [0, 1]] - box3d_corner[:, 7, [0, 1]]) ** 2,
                axis=1,
                keepdims=True,
            )
        )
        + np.sqrt(
            np.sum(
                (box3d_corner[:, 5, [0, 1]] - box3d_corner[:, 6, [0, 1]]) ** 2,
                axis=1,
                keepdims=True,
            )
        )
    ...
    theta = (
        np.arctan2(
            box3d_corner[:, 2, 1] - box3d_corner[:, 1, 1],
            box3d_corner[:, 2, 0] - box3d_corner[:, 1, 0],
        )
        + np.arctan2(
            box3d_corner[:, 3, 1] - box3d_corner[:, 0, 1],
            box3d_corner[:, 3, 0] - box3d_corner[:, 0, 0],
        )
        + np.arctan2(
            box3d_corner[:, 2, 0] - box3d_corner[:, 3, 0],
            box3d_corner[:, 3, 1] - box3d_corner[:, 2, 1],
        )
        + np.arctan2(
            box3d_corner[:, 1, 0] - box3d_corner[:, 0, 0],
            box3d_corner[:, 0, 1] - box3d_corner[:, 1, 1],
        )
    )[:, np.newaxis] / 4
